...
// in BPS Pro if the Plugin Firewall .htaccess file does not exist then add the ip address
function bpsPro_maintenance_mode_preview_ip() {
	...
	if ( ! file_exists($PluginsHtaccess) && file_exists($denyall_htaccess_file) ) {
...
if ( isset( $_POST['Submit-Maintenance-Mode-Form'] ) && current_user_can('manage_options') ) {
	...
	$bps_maintenance_values_ARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/bps-maintenance-values.php';
...
.'# END BPS MAINTENANCE MODE'."\n".'?>';
	...
	if ( is_multisite() && $blog_id != 1 ) {
		...
		if ( @copy($bps_maintenance_file, $subsite_maintenance_file) ) {
			...
			@copy($bps_maintenance_file, $subsite_maintenance_file_ARQ);	
		}
		...
		if ( file_put_contents( $subsite_maintenance_file, $stringReplaceMaint ) ) {
			@copy($subsite_maintenance_file, $subsite_maintenance_file_ARQ);
		}

		@copy($bps_maintenance_values, $bps_maintenance_values_network);
		@copy($bps_maintenance_values, $bps_maintenance_values_network_ARQ);
		...
		if ( file_put_contents( $bps_maintenance_values_network, $stringReplace ) ) {
			@copy( $bps_maintenance_values_network, $bps_maintenance_values_network_ARQ );
		...
		if ( file_put_contents( $bps_maintenance_values, $stringReplace ) ) {
			@copy($bps_maintenance_values, $bps_maintenance_values_ARQ);
...
<div id="MMode-button-position" style="">
...
$root_index_fileARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/index.php';
...
$MMindexARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/maintenance-mode-index.php';
$bps_maintenance_file = WP_PLUGIN_DIR . '/bulletproof-security/admin/htaccess/bps-maintenance.php';
$bps_maintenance_values = WP_PLUGIN_DIR . '/bulletproof-security/admin/htaccess/bps-maintenance-values.php';
$bps_maintenance_file_ARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/bps-maintenance.php';
$bps_maintenance_values_ARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/bps-maintenance-values.php';
$root_folder_maintenance = ABSPATH . 'bps-maintenance.php';
$root_folder_maintenance_values = ABSPATH . 'bps-maintenance-values.php';
$root_folder_maintenanceARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/bps-maintenance.php';
$root_folder_maintenance_valuesARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/bps-maintenance-values.php';
...
$format_error_2 = '/,[^\s]/'; // no whitespaces between commas
	...
	if ( preg_match($pattern, $stringReplace, $matches ) ) {
		...
		if ( !strpos($index_contents, "BPS MAINTENANCE MODE IP") ) {
			...
			@copy($bps_maintenance_values, $bps_maintenance_values_ARQ);
			@copy($bps_maintenance_values, $root_folder_maintenance_valuesARQ);
			...
			@copy($bps_maintenance_file, $bps_maintenance_file_ARQ);
			@copy($bps_maintenance_file, $root_folder_maintenanceARQ);	

			// first, second, third clicks...
			@copy($MMindexMaster, $MMindexARQ);
			@copy($MMindexMaster, $root_index_fileARQ);
	...
	$MMAllowFromTXTARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/maintenance-mode.txt';
	$wpadminHtaccess = ABSPATH . 'wp-admin/.htaccess';
	$wpadminHtaccessARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-admin/wpadmin.htaccess';
	...
	if ( $MMoptions['bps_maint_ip_allowed'] != '' && $MMoptions['bps_maint_backend'] == '1' ) {
		...
		@copy($MMAllowFromTXT, $MMAllowFromTXTARQ);
		@copy($wpadminHtaccess, $wpadminHtaccessARQ);
...
$MMindexARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/maintenance-mode-index-MU.php';
...
$subsite_maintenance_values_ARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/bps-maintenance-values-'.$subsite_remove_slashes.'.php';
$subsite_root_folder_maintenance = ABSPATH . 'bps-maintenance-'.$subsite_remove_slashes.'.php';
$subsite_root_folder_maintenanceARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/bps-maintenance-'.$subsite_remove_slashes.'.php';
$subsite_root_folder_maintenance_values = ABSPATH . 'bps-maintenance-values-'.$subsite_remove_slashes.'.php';
$subsite_root_folder_maintenance_valuesARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/bps-maintenance-values-'.$subsite_remove_slashes.'.php';
...
$format_error_2 = '/,[^\s]/'; // no whitespaces between commas
	...
	// the same index master file is used for all sites, each subsite will string replace its ip address array and copy the index file to the root folder again
		...
		if ( is_multisite() && ! is_subdomain_install() ) {
			...
			if ( ! preg_match( '/\$'.$subsite_subdomain_variable.'_status = \'(.*)\';/', $stringReplace, $matches ) ) {	
			
				$stringReplace = preg_replace('/#\sEND\sSUBSITE\sSTATUS/', "\$$subsite_subdomain_variable".'_status'." = 'On';\n# END SUBSITE STATUS", $stringReplace);
		
			} else {
		
				$stringReplace = preg_replace( '/\$'.$subsite_subdomain_variable.'_status = \'(.*)\';/', "\$$subsite_subdomain_variable".'_status'." = 'On';", $stringReplace);		
			}		

			// Create the subsite root domain in maintenance-mode-index-MU.php if it does not already exist
			if ( ! preg_match( '/\$'.$subsite_subdomain_variable.' = \''.$subsite_subdomain_var_value.'\';/', $stringReplace, $matches ) ) {	
	
				$stringReplace = preg_replace('/#\sEND\sSUBSITE\sURI/', "\$$subsite_subdomain_variable = '$subsite_subdomain_var_value';\n# END SUBSITE URI", $stringReplace);
			}	
			...
			$stringReplace = preg_replace('/default:(\s*(.*)){5}\s*#\sEND\sBPS\sSWITCH\s*\}/', "# BEGIN $subsite_remove_slashes CASE\n	case \$$subsite_subdomain_variable:\n		# BEGIN BPS MAINTENANCE MODE $subsite_remove_slashes IP\n		\$bps_maintenance_ip = array('127.0.0.1');\n		# END BPS MAINTENANCE MODE $subsite_remove_slashes IP\n		if ( \$all_sites == '1' || \$all_subsites == '1' ) {\n		require( dirname( __FILE__ ) . '/bps-maintenance.php' );\n		} else {\n		if ( in_array( \$_SERVER['REMOTE_ADDR'], \$bps_maintenance_ip ) || in_array( \$matches_three[0], \$bps_maintenance_ip ) || in_array( \$matches_two[0], \$bps_maintenance_ip ) || in_array( \$matches_one[0], \$bps_maintenance_ip ) || \$$subsite_subdomain_variable".'_status'." == 'Off' ) {\n		require( dirname( __FILE__ ) . '/wp-blog-header.php' );\n		} else {\n		require( dirname( __FILE__ ) . '/bps-maintenance-$subsite_remove_slashes.php' );\n		}\n		}\n		break;\n 	# END $subsite_remove_slashes CASE\n	default:\n		if ( \$all_sites == '1' || \$all_subsites == '1' ) {\n		require( dirname( __FILE__ ) . '/bps-maintenance.php' );\n		} else {\n		require( dirname( __FILE__ ) . '/wp-blog-header.php' );\n		}\n	# END BPS SWITCH\n	}", $stringReplace );
			}
		}
		...
		$stringReplace = preg_replace( $subsite_case_ip_pattern, "# BEGIN BPS MAINTENANCE MODE $subsite_remove_slashes IP\n		".'$bps_maintenance_ip'." = array('".str_replace(', ', "', '", $MMoptions['bps_maint_ip_allowed'])."');\n		# END BPS MAINTENANCE MODE $subsite_remove_slashes IP", $stringReplace);			
			...
			if ( file_put_contents($MMindexMaster, $stringReplace) ) {
				...
				@copy($subsite_maintenance_values, $subsite_maintenance_values_ARQ);
				@copy($subsite_maintenance_values, $subsite_root_folder_maintenance_valuesARQ);
				@copy($bps_maintenance_values, $root_folder_maintenance_values);
				@copy($bps_maintenance_values, $bps_maintenance_values_ARQ);
				@copy($bps_maintenance_values, $root_folder_maintenance_valuesARQ);				
				...
				@copy($subsite_maintenance_file, $subsite_maintenance_file_ARQ);
				@copy($subsite_maintenance_file, $subsite_root_folder_maintenanceARQ);	
				@copy($bps_maintenance_file, $root_folder_maintenance);
				@copy($bps_maintenance_file, $bps_maintenance_file_ARQ);
				@copy($bps_maintenance_file, $root_folder_maintenanceARQ);	
...
$gwiod_MMindexARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/wp-content/plugins/bulletproof-security/admin/htaccess/maintenance-mode-index-GWIOD.php';
	...
	if ( $publicly_displayed_url != $actual_wp_install_url ) {
		...
		if ( preg_match($gwiod_pattern, $gwiod_stringReplace, $matches ) ) {
			...
			if ( file_put_contents($gwiod_MMindexMaster, $gwiod_stringReplace) ) {
				...
				@copy($gwiod_MMindexMaster, $gwiod_MMindexARQ);
				@copy($gwiod_MMindexMaster, $gwiod_root_index_file);
				
				if ( $Flockoptions['bps_lock_index_php_gwiod'] == 'yes') {			
...
<?php
// Maintenance Mode - Frontend MMODE Turn Off used in Turn On function - Single & Single GWIOD
...
$root_folder_maintenance_valuesARQ = WP_CONTENT_DIR . '/bps-backup/autorestore/root-files/bps-maintenance-values.php';
	...
	if ( file_exists($root_index_file_backup) ) {
		...
		@copy($root_index_file_backup, $root_index_fileARQ);
		
		if ( @copy($root_index_file_backup, $root_index_file) ) {
	
			$delete_files = array($root_folder_maintenance, $root_folder_maintenance_values, $root_folder_maintenanceARQ, $root_folder_maintenance_valuesARQ);
			...
			// Network Subsites
				...
				if ( is_multisite() && ! is_subdomain_install() ) {
					...
					if ( preg_match( '/\$'.$subsite_subdomain_variable.'_status = \'(.*)\';/', $stringReplace, $matches ) ) {	
						...
						$stringReplace = preg_replace( '/\$'.$subsite_subdomain_variable.'_status = \'(.*)\';/', "\$$subsite_subdomain_variable".'_status'." = 'Off';", $stringReplace);		
					}			
				}
				...
				$delete_files = array( $subsite_root_folder_maintenance, $subsite_root_folder_maintenance_values, $subsite_root_folder_maintenanceARQ, $subsite_root_folder_maintenance_valuesARQ );
...
// Maintenance Mode - Turn Off - Single & Single GWIOD
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="bps-help_faq_table">
  ...
  <tr>
    ...
    <td class="bps-table_cell_help_links"><a href="http://www.ait-pro.com/bulletproof-security-pro-flash/bulletproof.html" target="_blank"><?php _e('BPS Pro Features', 'bulletproof-security'); ?></a></td>
  </tr>
  <tr>
 ...
 <tr>
    <td class="bps-table_cell_help_links"><a href="http://forum.ait-pro.com/forums/topic/maintenance-mode-guide-read-me-first/" target="_blank"><?php _e('Maintenance Mode Guide', 'bulletproof-security'); ?></a></td>
