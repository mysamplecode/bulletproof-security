...
<h2 style="margin-left:220px;"><?php _e('BulletProof Security Pro ~ System Info', 'bulletproof-security'); ?></h2>
...
function bps_get_proxy_real_ip_address() {
	...
	echo __('Uploads Folder', 'bulletproof-security').': <strong>';
	echo str_replace( ABSPATH, '', $wp_upload_dir['basedir'] ).'</strong>';	
	}

	if ( is_multisite() && is_dir( ABSPATH . @UPLOADBLOGSDIR ) ) {
	echo '<br>'.__('blogs.dir Uploads Folder', 'bulletproof-security').': <strong>';
	echo @UPLOADBLOGSDIR.'</strong>';
	}
	// check UPLOADS CONSTANT = wp-content/blogs.dir/1/files = default / not really defined
	if ( is_multisite() && defined( 'UPLOADS' ) ) {
	echo '<br>'.__('UPLOADS Constant', 'bulletproof-security').': <strong>';
	echo str_replace( trailingslashit( WP_CONTENT_FOLDERNAME ), '', untrailingslashit( UPLOADS ) ).'</strong>';
	}

	echo '<br>'.__('WP Permalink Structure', 'bulletproof-security').': <strong>';
...
<?php 
	...
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/autorestore", "705");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/logs", "705");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/master-backups", "705");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/quarantine", "705");
	...
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/autorestore", "755");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/logs", "755");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/master-backups", "755");
	bps_check_perms("../$bps_wpcontent_dir/bps-backup/quarantine", "755");
	if ( $DBBoptions['bps_db_backup_folder'] != '' ) {
